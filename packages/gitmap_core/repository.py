...
class Repository:
    ...
    def _revert_layers(
        ...
        current_by_id = {layer.get("id"): layer for layer in current_layers if layer.get("id")}
        commit_by_id = {layer.get("id"): layer for layer in commit_layers if layer.get("id")}
        parent_by_id = {layer.get("id"): layer for layer in parent_layers if layer.get("id")}
    ...
    def _apply_layer_changes(
        ...
        for layer_id, layer in commit_by_id.items():
            if layer_id in parent_by_id and commit_by_id[layer_id] != parent_by_id[layer_id]:
                ...
                if layer_id in current_by_id:
                    ...
                    for i, existing_layer in enumerate(result):
                        if existing_layer.get("id") == layer_id:
                            result[i] = layer
                            break
        ...
        for layer_id in parent_by_id:
            if layer_id not in commit_by_id:
                ...
                result = [layer for layer in result if layer.get("id") != layer_id]
