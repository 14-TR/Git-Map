...
# ---- MapDiff.to_dict Tests ---------------------------------------------------------------------------------


class TestMapDiffToDict:
    """Tests for MapDiff.to_dict() serialization method."""
    ...
    def test_empty_diff_to_dict(self) -> None:
        ...
        assert result["property_changes"] == {}

    def test_added_layer_to_dict(self) -> None:
        """Added layer appears in serialized output."""
        map_diff = MapDiff(
            layer_changes=[
                LayerChange(
                    layer_id="l1",
    ...
    def test_modified_layer_with_details_to_dict(self) -> None:
        """Modified layer includes details in serialized output."""
        map_diff = MapDiff(
            layer_changes=[
                LayerChange(
                    layer_id="l2",
                    layer_title="Parcels",
                    change_type="modified",
                    details={"values_changed": {"opacity": {"old": 0.8, "new": 1.0}}},
                ),
            ]
        )
        result = map_diff.to_dict()
    ...
    def test_mixed_changes_to_dict(self) -> None:
        """Mixed layer + table changes serialize correctly."""
        map_diff = MapDiff(
            layer_changes=[
                LayerChange("l1", "Roads", "added"),
                LayerChange("l2", "Parcels", "removed"),
                LayerChange("l3", "Buildings", "modified", details={"x": 1}),
            ],
            table_changes=[
                LayerChange("t1", "Owners", "added"),
            ],
            property_changes={"baseMap": "changed"},
        )
        result = map_diff.to_dict()
        assert result["has_changes"] is True
        assert result["stats"]["added"] == 2  # 1 layer + 1 table
    ...
    def test_to_dict_is_json_serializable(self) -> None:
        ...
        map_diff = MapDiff(
            layer_changes=[
                LayerChange("l1", "Roads", "added"),
                LayerChange("l2", "Parcels", "modified", details={"a": [1, 2]}),
            ],
        )
        result = map_diff.to_dict()
